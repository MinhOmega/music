...
export default function ConnectDrawer() {
  ...
  const { showDrawer, connected, roomConnected, username, isHost, hostId, mode, connections } = useAppSelector(
    (state) => state.connect
  )
    ...
    >
      ...
      <div className={styles.peerInfo}>
        <div className={styles.label}>
          <Typography.Text className={styles.text}>Your ID</Typography.Text>
          <Badge
      ...
      {!roomConnected && (
        <Card className={styles.connectCard} bodyStyle={{ padding: 0 }}>
          <Tabs
            tabBarStyle={{
              paddingLeft: 16,
              paddingRight: 16,
            ...
            items={[
              {
                ...
                children: (
                  <div className={styles.tabContent}>
                    <Typography.Text>Join a room by connect to host's ID.</Typography.Text>
                    <Form className={styles.connectForm} onFinish={doConnect}>
                      <Form.Item
                        name={'id'}
                        required
                        rules={[
                ...
                children: (
                  <div className={styles.tabContent}>
                        ...
                        message={
                          <>
                            {createMode === 'broadcast' ? (
                              <small>This mode prevent members from controlling the player.</small>
                            ) : (
                              <small>This mode allow members to control the player simultaneously.</small>
                            )}
                          </>
                        }
                      />
                    ...
                    <div className={styles.createRoomForm}>
                      <Select value={createMode} onChange={(value) => setCreateMode(value as ConnectMode)}>
