  ...
  return (
    <div className={styles.playlistImageWrapper}>
      <div
        ...
        style={{
          backgroundImage:
            tracks.length > 0 ? `url('${(tracks[0].thumbnails || tracks[0].thumbnail)[0].url}')` : undefined,
        }}
      />
      {playlist.tracks.length === 0 && <HeartFilled />}
    </div>
  )
}
...
export default function PlaylistsSection() {
  ...
  return (
    <div>
      ...
      />
      <div ref={ref} className={styles.listOuter}>
        {renderPlaylists.map((playlist) => {
          let playlistDescription = ''
          if (playlist.tracks.length > 0) {
            playlistDescription = playlist.tracks
              .filter((x, i) => i < 2)
              .map((t) => t.author.name)
              .join(', ')
          }
          return (
            ...
            >
              ...
              <div className={styles.meta}>
                <Typography.Text className={styles.name}>{playlist.name}</Typography.Text>
